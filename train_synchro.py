...
parameter_servers = ["10.24.1.221:2226","10.24.1.222:2227"]
workers = ["10.24.1.223:2228", "10.24.1.224:2229"]
...
tf.app.flags.DEFINE_string('model_dir','/scratche/home/swapnil/courses/model_sync/','path to store model')
#tf.app.flags.DEFINE_string('summary_dir','./summary','path to store summary_dir')


LOG_DIR = '/scratche/home/swapnil/courses/sync_logs'
...
def main(_):
	...
	if FLAGS.job_name == "ps":
		...
		with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index, cluster=cluster)):
			...
			optimizer = tf.train.SyncReplicasOptimizer(optimizer,replicas_to_aggregate=2,total_num_replicas=2,use_locking=True)
				...
				for one_epoch in range(FLAGS.max_epoch):
					...
					for i in range(int(len(train_order)/float(FLAGS.batch_size))):
						...
						if FLAGS.task_index == 0:
                                                        time_dist_w1.append([local_step,current_step,time.time()-Start])
                                                elif FLAGS.task_index == 1:
                                                        time_dist_w2.append([local_step,current_step,time.time()-Start])
					
					if FLAGS.task_index == 0:
                                                #st_time_dist_w1.append(time.time() - Start)
                                                save_cost_w1.append([batch_lossnn,batch_losskg,batch_lossnn + 0.01*batch_losskg,s1/tot1])
                                                np.save("/scratche/home/swapnil/courses/time_dist_w1.npy",time_dist_w1)
                                                np.save("/scratche/home/swapnil/courses/save_cost_w1.npy",save_cost_w1)
                                                print(time_dist_w1[len(time_dist_w1)-1])
                                        elif FLAGS.task_index == 1:
                                                #st_time_dist_w2.append(time.time() - Start)
                                                save_cost_w2.append([batch_lossnn,batch_losskg,batch_lossnn + 0.01*batch_losskg,s1/tot1])
                                                np.save("/scratche/home/swapnil/courses/time_dist_w2.npy",time_dist_w2)
                                                np.save("/scratche/home/swapnil/courses/save_cost_w2.npy",save_cost_w2)
                                                print(time_dist_w2[len(time_dist_w2)-1])
