...
parameter_servers = ["10.24.1.213:2222","10.24.1.214:2223"]
workers = ["10.24.1.215:2224", "10.24.1.216:2225"]
...
tf.app.flags.DEFINE_string('model_dir','./model_stale/','path to store model')
...
def main(_):
	...
	if FLAGS.job_name == "ps":
		...
		with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index, cluster=cluster)):
			...
			stale_param = tf.placeholder(dtype=tf.int32, shape = [], name='stale_param')
			...
			optimizer = tf.contrib.opt.DropStaleGradientOptimizer(optimizer,staleness=stale_param,use_locking=True)
			...
			# optimizer_kg = tf.train.SyncReplicasOptimizer(optimizer_kg,replicas_to_aggregate=2,total_num_replicas=2,use_locking=True)
			...
			def train(sess,local_step,stale):
				...
				def train_step(head, tail, word, pos1, pos2, mask, leng, label_index, label, scope, weights, pos_h_batch, pos_t_batch, pos_r_batch, neg_h_batch, neg_t_batch, neg_r_batch, stale):
					feed_dict = {
				...
				for one_epoch in range(FLAGS.max_epoch):
					...
					for i in range(int(len(train_order)/float(FLAGS.batch_size))):
						...
						output, batch_losskg, batch_lossnn, correct_predictions = train_step(train_head[index], train_tail[index], train_word[index,:], train_pos1[index,:], train_pos2[index,:], train_mask[index,:], train_len[index],train_label[index], label_, np.array(scope), weights, ph, pt, pr, nh, nt, nr, stale)
						local_step += 1
						num = 0
						s = 0
						...
						if FLAGS.task_index == 0:
                                                        st_time_dist_w1.append([local_step,current_step,time.time()-Start])
                                                elif FLAGS.task_index == 1:
                                                        st_time_dist_w2.append([local_step,current_step,time.time()-Start])
						
					if FLAGS.task_index == 0:
	                    			#st_time_dist_w1.append(time.time() - Start)
	                  	  		st_save_cost_w1.append([batch_lossnn,batch_losskg,batch_lossnn + 0.01*batch_losskg,s1/tot1])
						np.save("st_time_dist_w1.npy",st_time_dist_w1)
                                        	np.save("st_save_cost_w1.npy",st_save_cost_w1)
	                    			print(st_time_dist_w1[len(st_time_dist_w1)-1])
	                		elif FLAGS.task_index == 1:
	                			#st_time_dist_w2.append(time.time() - Start)
	                			st_save_cost_w2.append([batch_lossnn,batch_losskg,batch_lossnn + 0.01*batch_losskg,s1/tot1])
						np.save("st_time_dist_w2.npy",st_time_dist_w2)
                                        	np.save("st_save_cost_w2.npy",st_save_cost_w2)
	                    			print(st_time_dist_w2[len(st_time_dist_w2)-1])
		...
		#syn_replicas_nn_hook = optimizer.make_session_run_hook((FLAGS.task_index == 0))
		# syn_replicas_kg_hook = optimizer_kg.make_session_run_hook(is_chief)

		stop_hook = tf.train.StopAtStepHook(last_step = 1600*(FLAGS.max_epoch+2))		
		#summary_hook = tf.train.SummarySaverHook(save_secs = 600, output_dir = LOG_DIR, summary_op = merged_summary)
		#hook = [syn_replicas_nn_hook,stop_hook]
		hook = [stop_hook]
